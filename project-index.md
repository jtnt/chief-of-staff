...
**Last synced:** 2026-02-07 (active knowledge engagement protocol)
...
**Recent Work:**
- 2026-02-07: **Active knowledge engagement protocol** - Operationalized the knowledge system for active use during conversation. Implemented behavioral protocol with four concrete behaviors: (1) thread activation (load 2-3 depth docs when conversation enters a thread's territory), (2) enriched participation (use loaded depth to surface insights and connections), (3) staging capture (write novel thinking to working/session-staging.md during conversation), (4) proactive promotion (Claude invokes /learn when staging has substance). Updated /learn skill to be model-invocable and accept staged content. Tracked .claude/ directory in git (skills and commands are project knowledge, not machine-specific). Updated documentation. Key pattern: infrastructure ≠ usage — behavioral protocols activate infrastructure by specifying triggers, actions, transparency requirements, and judgment boundaries. Session pattern extracted for global CLAUDE.md.
...
- 2026-02-06: **Vercel research consolidation** - Resolved duplicate research files on Vercel AGENTS.md evaluation by comparing analytical depth vs reference capture. Merged strategic insights (three failure points mechanism, creation bottleneck validation, third option positioning) from Working file into project-knowledge.md Vercel section. Key preserved insights: simple delivery + good structure beats complex infrastructure; index-plus-retrieval as third option between pre-determined composition and agentic search. Research file remains clean citation source (100% pass rate data, eval methodology), Working file marked as absorbed.
...
**Last synced:** 2026-02-06 (CLAUDE.md updates & training index)
...
- 2026-02-06: **CLAUDE.md accuracy updates & training materials organization** - Corrected project overview to properly distinguish More Vang (print/marketing) from Mythos (higher ed SaaS). Created comprehensive INDEX.md for 4-session training materials (64+ files) with session breakdowns and usage guidance. Non-git project so changes exist in working directory only.
